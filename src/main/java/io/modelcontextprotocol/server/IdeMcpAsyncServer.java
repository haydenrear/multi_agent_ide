...
//    TODO: override toolsListRequestHandler so that tools list call takes into account the user's username, only returning specific tools registered.
    IdeMcpAsyncServer(McpServerTransportProvider mcpTransportProvider, ObjectMapper objectMapper, McpServerFeatures.Async features,
    ...
    private List<ResourceTemplate> getResourceTemplates() {
        ...
        List<ResourceTemplate> resourceTemplates = this.resources.keySet()
                ...
                .map(uri -> {
                    ...
                    var template = new ResourceTemplate(resource.uri(), resource.name(), resource.description(),
