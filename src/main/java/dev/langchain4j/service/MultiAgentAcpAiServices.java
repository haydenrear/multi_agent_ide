...
public class MultiAgentAcpAiServices<T> extends AiServices<T> {
    ...
    public MultiAgentAcpAiServices(AiServiceContext context) {
        super(context);

        this.chatRequestTransformer = (cr, mem) -> cr.toBuilder()
                .parameters(
                        MultiAgentIdeChatRequestParameters.builder()
                                .overrideWith(cr.parameters())
                                .memoryId(mem)
                                .build()
                )
                .build();
    ...
    public AiServices<T> chatRequestTransformer(UnaryOperator<ChatRequest> chatRequestTransformer) {
        context.chatRequestTransformer = (req, memId) -> {
            var updatedRequest = chatRequestTransformer.apply(req);
            var afterChange = this.chatRequestTransformer.apply(updatedRequest, memId);
            return afterChange;
        };
        return this;
    }
    ...
    private Object handleChatMemoryAccess(Method method, Object[] args) {
        return switch (method.getName()) {
            ...
            case "evictChatMemory" ->
                    context.chatMemoryService.evictChatMemory(args[0]) != null;
            default ->
        ...
        Object proxyInstance = Proxy.newProxyInstance(
                ...
                new InvocationHandler() {
                    ...
                    public Object invoke(Method method, Object[] args, InvocationContext invocationContext) {
                        ...
                        dev.langchain4j.data.message.UserMessage userMessage = prepareUserMessage(method, args, userMessageTemplate, variables);
                        ...
                        if (maybeContents.isPresent()) {
                            ...
                            userMessage = dev.langchain4j.data.message.UserMessage.from(userMessage.name(), allContents);
                        }
    ...
    private dev.langchain4j.data.message.UserMessage invokeInputGuardrails(
